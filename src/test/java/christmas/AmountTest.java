...
class AmountTest {
    ...
    void test_correct_menu_and_count_price() {
        int steak = TotalAmount.calculateUserMenuAndPrice("티본스테이크", 2);
        assertEquals(110000, steak);
        int tapas = TotalAmount.calculateUserMenuAndPrice("타파스", 3);
    ...
    void test_when_invalid_menu(){
        int steak = TotalAmount.calculateUserMenuAndPrice("양갈비스테이크", 2);
    ...
    void test_multiple_correct_menu_and_count_price() {
        ...
        menuMap.put("초코케이크", 2);
        menuMap.put("제로콜라", 1);
        TotalAmount totalPrice = TotalAmount.totalPriceBeforeSale(menuMap);
